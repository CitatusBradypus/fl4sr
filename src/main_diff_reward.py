...
dict_list = {'list_reward': 2}
dict_update_period = {'updatePeriod': [1,3, 5, 10]}
    ...
    for uP in dict_update_period['updatePeriod']:
        for rmax in dict_reward_max_collision['reward_max_collision']:
            for algo in dict_algorithms['algorithms']:
                COMMAND_LIST.append(['rosrun', 'fl4sr', 'experiment_limit.py', algo, f'--mode={"learn"}',  f'--updatePeriod={uP}', f'--reward_goal={rg}',\
                 f'--reward_collision={rc}',f'--reward_progress={rp}', f'--reward_max_collision={rmax}', f'--list_reward={dict_list["list_reward"]}', '--factor_linear=0.25', f'--discount_factor={0.99}'])

# for rg, rc, rp in zip(dict_reward['reward_goal'], dict_reward['reward_collision'], dict_reward['reward_progress']):
#     for uP in dict_update_period['updatePeriod']:
#         for rmax in dict_reward_max_collision['reward_max_collision']:
#             for algo in dict_algorithms['algorithms']:
#                 COMMAND_LIST.append(['rosrun', 'fl4sr', 'experiment.py', algo, f'--mode={"learn"}', f'--updateStep={True}', f'--updatePeriod={uP}', f'--reward_goal={rg}',\
#                  f'--reward_collision={rc}',f'--reward_progress={rp}', f'--reward_max_collision={rmax}', f'--list_reward={dict_list["list_reward"]}', '--factor_linear=0.25', f'--discount_factor={0.99}'])
